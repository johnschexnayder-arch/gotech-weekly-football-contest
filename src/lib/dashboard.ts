...
if (openWeekError) {
  throw new Error(openWeekError.message);
}
...
// If no week is currently open,
// use the most recent week instead.
if (!week) {
  ...
  if (recentWeekError) {
    throw new Error(recentWeekError.message);
  }
...
if (!week) {
  ...
  const totalGames = dbGames.length;
